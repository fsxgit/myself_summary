...
class AssetController extends AdminbaseController
{
    ...
    public function plupload ()
    {
        ...
        $filetypes = array('image' => array('title' => 'Image files','extensions' => $upload_setting['image']['extensions']),
        ...
        if (IS_POST)
        {
            ...
            if ($info)
            {
                ...
                $this->thinkImage(C("TMPL_PARSE_STRING.__UPLOAD__") . $savepath . $first['savename']);
                $http = sp_get_host();
                $this->ajaxReturn(
                        array('preview_url' => $http . $preview_url,
                                'filepath' => $filepath,
                                'url' => $url,
                                'name' => $oriName,
                                'status' => 1,

                ...
                <div class="bang-cel-box">
					...
					<?php if($result['linkMan'] == ""){ ?>
                        <div>
							...
							<input type="text" class = "ggg" id = "ianxiren" value="<?php echo ($result["linkMan"]); ?>" placeholder="请输入联系人姓名"/>
							</p>
					...
					<?php } ?>
                        <p>
							...
							<span class="cel-has"><?php echo ($result["phone"]); ?></span>
							<a href="<?php echo U('Portal/pinfo/cktel');?>" class="xg-tel">修改</a>
						</p>
					...
					<?php }else if($result['address'] != ""){ ?>
						<div>
							<p>
							<span class="tit">通讯地址</span>
							<input class = "ggg" type="text" id = "dizhi" value="<?php echo ($result["address"]); ?>" placeholder="请输入通讯地址"/>
							</p>
							<p class="warn-box" id = "dizhits"></p>
						</div>
					<?php } ?>
...
//提示错误后，再获取焦点是，提示隐藏
	...
	function formck(){
		...
		$.ajax({
            type: "POST",
            url: "<?php echo U('Portal/pinfo/lianxiren1');?>",

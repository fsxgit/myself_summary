    ...
    <div class="main-hou">
        <div class="brand-gl c333">
           <div class="bgl-nav bb">
                ...
                <a href="<?php echo U('Brand/brand_query_lst');?>" class="active">查询列表</a>
            </div>

        </div>
		<form method="post" action="<?php echo U('Brand/brand_query_lst');?>">
...
<input type="text" id="demo1" class="sui-form time-input" name="start_time" placeholder="开始日期"/>
		...
		<div id="mydiv">
        <!--/*提示说明*//-->  
		<?php if( $user["isclaimed"] == 0 ): ?><div class="c-rl-cont-ts cx-list-ts">
			 <p>您还没有认领公司，最多可免费查询三次，认领公司后不限制 查询次数<?php echo ($user["chasum"]); ?>。<a class="kan fc1 fb">去认领公司</a></p>
        </div><?php endif; ?>
        <!--/*提示说明* end//-->
        <!--商标-->
        <div class="brand-box">
            <table class="brand-table c333">
                <tr class="bt-tr1 bg5 c999">
                    ...
                    <td style="width:16.6%;">查询单号</td>
                    <td style="width:8.3%;">商标图样</td>
                    <td style="width:13.1%;">商标名称</td>
                    <td style="width:10.6%;">查询方式</td>
                    <td style="width:12.5%;">查询日期</td>
					<td style="width:12.5%;">查询结果日期</td>
				...
				<?php if(is_array($arr)): foreach($arr as $key=>$vo): ?><tr>
                    ...
                    <td class="brand-logo"><img src="<?php echo ($vo["markpicture"]); ?>" alt=""/></td>
                    <td><?php if($vo['newmarkname'] == '' ): echo ($vo["markname"]); else: echo ($vo["newmarkname"]); endif; ?></td>
                    <td><?php if($vo["type"] == 1): ?>智能推荐<?php elseif($vo["type"] == 2): ?>委托顾问查询<?php elseif($vo["type"] == 3): ?>自主选择类别<?php endif; ?></td>
                    <td><?php echo date('Y-m-d H:i:s',$vo['addtime']);?></td>
					<td>
					<?php if($vo["endtime"] == '' ): ?>--<?php else: echo date('Y-m-d H:i:s',$vo['endtime']); endif; ?></td>
                    <td><?php if($vo["state"] == 0): ?>查询中<?php elseif($vo["state"] == 1): ?>全部可注册<?php elseif($vo["state"] == 2): ?>部分可注册<?php elseif($vo["state"] == 3): ?>全部不可注册<?php elseif($vo["state"] == 4): ?>查询失败<?php endif; ?></td>
                    <td><a href="<?php echo U('Brand/brand_query_details',array('queryid'=>$vo['queryid']));?>">查看</a></td>

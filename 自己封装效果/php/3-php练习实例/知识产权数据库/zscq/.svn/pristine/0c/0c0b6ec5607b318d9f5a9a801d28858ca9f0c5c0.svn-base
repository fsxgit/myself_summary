...
class WordpdfController extends CommonController {
	...
	public function tihuan(){
		...
		$document->setValue('value9',$a9);
                ...
                $document->setValue('nian','2014');
                $document->setValue('yu','11');
                $document->setValue('$$','11');
        ...
        public function shangbiaozhuceweituo(){
            ...
            $youbian = M("youbian")->select();
            foreach($youbian as $k=>$v){
                if(strstr($arr1['applicantaddress'],$v['a']))
                {
                    $code = $v['c'];
                }
            }
            ...
            //处理数据
            $a1 = $this->ti($arr1['applicantname'],40);
            $a2 = $this->ti($arr2['markname'],24);
            $a3 = $this->ti($arr1['applicantaddress'],28);
            $a4 = $this->ti($arr1['customername'],28);
            $a5 = $this->ti($arr1['customermobile'],28);
            $a6 = $this->ti($code,28);
            $time = date('Y-m-d',$arr1['paytime']);
            $ymd = explode('-',$time);
            //模板替换数据
            $document->setValue("value1",$a1);
            $document->setValue('value2',$a2);
            ...
            $document->setValue('value5',$a5);
            $document->setValue('value6',$a6);
            $document->setValue('nian',$ymd['0']);
            $document->setValue('yu',$ymd['1']);
            $document->setValue('$$',$ymd['2']);
            //本地测试不加./线上加
            ...
            $doc_file = "D:/WWW/zscq/WORD/shangbiaozhuceweituo.docx";
            //文件命名
            $pdf_file = "5.pdf";
            $output_file = $output_dir . $pdf_file;
            $doc_file = "file:///" . $doc_file;
            ...
            Header("Content-Disposition: attachment; filename="."转让移转申请注册商标申请书.pdf");
            $buffer=1024;
            while(!feof($fp)){
        ...
        public function zhuanrangshenqingweituo(){
            ...
            $a1 = $this->ti($arr1['applicantname'],40);
            $a2 = $this->ti($arr2['markname'],24);
            $a3 = $this->ti($arr1['applicantname'],28);
            $a4 = $this->ti($arr1['customername'],28);
            $a5 = $this->ti($arr1['customermobile'],28);
            $a6 = $this->ti($arr1['applicantname'],28);
            //模板替换数据
            $document->setValue("value1",$a1);
            $document->setValue('value2',$a2);
            $document->setValue('value3',$a3);
	...
	function str_suffix($str, $n=1, $char=" "){
        ...
        public function zhuanrangpl(){
            ...
            //本地测试不加./线上加
            $document->save('./WORD/shangbiaozhuanrang.docx');
            $wordname = "shangbiaozhuanrang.docx";
            return $wordname;
        }               
        public function zhuanrangpl1(){
            require 'simplewind/Lib/Util/PHPWord/PhpWord.php';
            $PHPWord = new \PHPWord();
            $document = $PHPWord->loadTemplate('simplewind/Lib/Util/PHPWord/Examples/shangbiaozhuanrangweituo.docx');
            //处理数据
            ...
            //本地测试不加./线上加
            $document->save('./WORD/shangbiaozhuanrangweituo.docx');
            $wordname1 = "shangbiaozhuanrangweituo.docx";
            return $wordname1;
        }
        
        public function ceshixxx(){
            $word2 = $this->zhuanrangpl1();
            $word1 = $this->zhuanrangpl();
            
            var_dump($word1);
            var_dump($word2);
            exit;
            //$this->xiazai(array('D:/WWW/zscq/PDF/'.$word1,'D:/WWW/zscq/PDF/'.$word2));
        }
}
